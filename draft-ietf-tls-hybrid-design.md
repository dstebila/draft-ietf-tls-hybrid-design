...
Note that the indistinguishability property of Diffie-Hellman is not indistinguishability from a random bit string, but instead indistinguishability from a random group element (where the group element is chosen from the subgroup that the DH operation is done on).  Because TLS uses a key derivation function to process the shared secrt, this distinction is unimportant.
